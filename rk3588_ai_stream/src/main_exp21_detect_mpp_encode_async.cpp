...
{   //参数校验
    if (argc != 8) {
        ...
        printf("Example: %s models/yolo11.rknn /dev/video11 1280 720 120 output/exp21_3_detect_mpp_encode/detect_120f_1280x720.h264 output/exp08_3_detect_fifo_mpp/profile.csv\n", argv[0]);
        return -1;
    }
    //atoi字符串转成整数，赋值给相应变量
    ...
    const int mpp_fps = 30;
    const int mpp_bitrate = 4000000;
    //计算图像大小
    ...
    std::thread encoder_thread([&]() {     // 创建新线程，线程函数可以引用外部变量
        ...
        while (true) {
            ...
            if (!local_packet.empty() && written != local_packet.size()) {
                async_encode_failures++;
                printf("[ENC] write failed at frame=%d written=%zu expected=%zu\n",
        ...
        printf("[ENC] encoder thread exit, encoded=%d failures=%d drops=%d\n",
               async_encoded_frames.load(),
    ...
    for (int frame_id = 0; frame_id < frames; ++frame_id) { //每一帧的推理主循环
        ...
        rga_buffer_t rga_dst_nv12 = wrapbuffer_virtualaddr((void*)out_nv12_buf.data(),
                                                           ...
                                                           height,
                                                           RK_FORMAT_YCbCr_420_SP);
        //RGB转换成NV12格式（底层编码器对NV12格式压缩效率最高）
        ...
        if (frame_id % 30 == 0) {
            printf("frame=%d total=%.3f fps=%.3f rga_in=%.3f model=%.3f draw=%.3f rga_out=%.3f queue_push=%.3f det=%d\n",
                   frame_id,
                   total_ms,
                   fps,
                   rga_in_ms,
    ...
    int async_count = async_encoded_frames.load();
    double async_avg_encode_ms = async_count > 0 ? (double)async_encode_us.load() / 1000.0 / async_count : 0.0;
    double async_avg_write_ms = async_count > 0 ? (double)async_write_us.load() / 1000.0 / async_count : 0.0;
    double async_avg_total_ms = async_count > 0 ? (double)async_total_us.load() / 1000.0 / async_count : 0.0;

    printf("async_encoded_frames : %d\n", async_count);
