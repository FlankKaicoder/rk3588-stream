...
#include <chrono>
#include <math.h>
static inline double yolo11_profile_diff_ms(const std::chrono::steady_clock::time_point &start,
...
int inference_yolo11_model(rknn_app_context_t *app_ctx, image_buffer_t *img, object_detect_result_list *od_results)
{
    ...
    if ((!app_ctx) || !(img) || (!od_results))
    {
            ...
            auto yolo11_prof_func_end = std::chrono::steady_clock::now();
            double yolo11_prof_total_ms = yolo11_profile_diff_ms(yolo11_prof_func_start, yolo11_prof_func_end);
        
            printf("[YOLO11_PROFILE] preprocess=%.3f inputs_set=%.3f rknn_run=%.3f outputs_get=%.3f postprocess=%.3f outputs_release=%.3f total=%.3f\n",
                   yolo11_prof_preprocess_ms,
                   yolo11_prof_inputs_set_ms,
                   yolo11_prof_rknn_run_ms,
                   yolo11_prof_outputs_get_ms,
                   yolo11_prof_postprocess_ms,
                   yolo11_prof_outputs_release_ms,
                   yolo11_prof_total_ms);
    ...
    yolo11_prof_preprocess_ms += yolo11_profile_diff_ms(yolo11_prof_stage_start, yolo11_prof_stage_end);
if (ret < 0)
    {
    ...
    yolo11_prof_stage_end = std::chrono::steady_clock::now();
    yolo11_prof_inputs_set_ms += yolo11_profile_diff_ms(yolo11_prof_stage_start, yolo11_prof_stage_end);
if (ret < 0)
    {
...
if (ret < 0)
    ...
    yolo11_prof_stage_end = std::chrono::steady_clock::now();
    yolo11_prof_outputs_get_ms += yolo11_profile_diff_ms(yolo11_prof_stage_start, yolo11_prof_stage_end);
if (ret < 0)
    {
    ...
    yolo11_prof_stage_end = std::chrono::steady_clock::now();
    yolo11_prof_postprocess_ms += yolo11_profile_diff_ms(yolo11_prof_stage_start, yolo11_prof_stage_end);
// Remeber to release rknn output
    ...
    yolo11_prof_stage_end = std::chrono::steady_clock::now();
    yolo11_prof_outputs_release_ms += yolo11_profile_diff_ms(yolo11_prof_stage_start, yolo11_prof_stage_end);
out:
    if (dst_img.virt_addr != NULL)
    {
        #if defined(DMA_ALLOC_DMA32)
