...
{   //参数校验
    ...
    for (int frame_id = 0; frame_id < frames; ++frame_id) {
        ...
        bool enc_ok = mpp_encoder.encode(out_nv12_buf.data(), nv12_size, h264_packet);
        if (!enc_ok) {
            printf("MppH264Encoder encode failed at frame=%d\n", frame_id);
            break;
        }

        size_t written = 0;
        if (!h264_packet.empty()) {
            written = fwrite(h264_packet.data(), 1, h264_packet.size(), fout);
        }
        auto t_write1 = std::chrono::steady_clock::now();

        if (!h264_packet.empty() && written != h264_packet.size()) {
            printf("short write frame=%d written=%zu expected=%zu\n",
                   frame_id, written, h264_packet.size());
            xioctl(fd, VIDIOC_QBUF, &buf);
            break;
        }
        //缓冲区重新入队（归还buffer）
        ...
        if (frame_id % 30 == 0) {
            printf("frame=%d total=%.3f fps=%.3f rga_in=%.3f model=%.3f draw=%.3f rga_out=%.3f mpp_write=%.3f det=%d\n",
                   frame_id,
                   total_ms,
                   fps,
                   rga_in_ms,
    ...
    printf("\n========== exp21-3 detect mpp encode result ==========\n");
    printf("frames              : %d\n", actual_frames);
    ...
    printf("avg_mpp_encode_write: %.3f\n", sum_write_ms / actual_frames);
