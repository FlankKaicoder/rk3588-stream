...
int main(int argc, char** argv) {
    if (argc != 7) {
        std::cerr << "Usage: " << argv[0]
                  << " <alsa_dev> <rate> <channels> <duration_sec> <period_frames> <out_csv>\n";
        return 1;
    }
    ...
    if (err < 0) {
        die_alsa("snd_pcm_open failed", err);
        return 2;
    }
    ...
    if (err < 0) {
        die_alsa("snd_pcm_hw_params failed", err);
    ...
    int tstamp_type_ret = snd_pcm_sw_params_set_tstamp_type(
        pcm, sw, SND_PCM_TSTAMP_TYPE_MONOTONIC);
    if (tstamp_type_ret < 0) {
        std::cerr << "warning: set monotonic timestamp type failed: "
                  << snd_strerror(tstamp_type_ret) << "\n";
    }
#endif

    err = snd_pcm_sw_params(pcm, sw);
    if (err < 0) {
        die_alsa("snd_pcm_sw_params failed", err);
    ...
    if (err < 0) {
        die_alsa("snd_pcm_prepare failed", err);
        snd_pcm_close(pcm);
        return 5;
    }
    ...
    std::cout << "audio timestamp csv: " << csv_path << "\n";
    return 0;
}
